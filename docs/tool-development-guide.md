...
3. 如果超过，自动持久化到 `~/.jobopx/sessions/<sessionId>/tool-results/<toolUseId>.txt`
4. 替换结果内容为预览消息
...
A: 持久化文件存储在 `~/.jobopx/sessions/<sessionId>/tool-results/` 目录中。建议定期清理：
- **手动清理**: 删除旧会话目录
- **自动清理**: 可以实现定期清理脚本，删除超过 7 天的会话目录
- **磁盘空间**: 监控 `~/.jobopx/sessions/` 目录大小，超过阈值时清理最旧的会话

**清理脚本示例**:
```bash
# 删除 7 天前的会话
find ~/.jobopx/sessions -type d -mtime +7 -exec rm -rf {} \;
```

## 持久化文件管理

### 存储位置
- **路径**: `~/.jobopx/sessions/<sessionId>/tool-results/`
